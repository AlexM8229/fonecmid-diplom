...
#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
	...
	Процедура ОбработкаПроведения(Отказ, РежимПроведения)
		...
		НачислитьПроцентЗаВыполненныеРаботы();
	...
	Процедура СформироватьДвиженияОтпуск()
		...
		Для Каждого ТекСтрокаНачисления Из Начисления Цикл
			...
			Если ТекСтрокаНачисления.ВидРасчета = ПланыВидовРасчета.ВКМ_ОсновныеНачисления.Отпуск Тогда
				...
				Движение.Показатель = (ТекСтрокаНачисления.ДатаОкончания - ТекСтрокаНачисления.ДатаНачала)/86400 + 1; 
	...
	Процедура РассчитатьОклад()
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			...
			Движение.Результат = ВыборкаДетальныеЗаписи.Показатель * ВыборкаДетальныеЗаписи.Факт / ВыборкаДетальныеЗаписи.План;
			...
			Движение.ПериодРегистрации = Дата;
			Движение.Сотрудник = ВыборкаДетальныеЗаписи.Сотрудник;
			Движение.Результат = ВыборкаДетальныеЗаписи.Показатель * ВыборкаДетальныеЗаписи.Факт / ВыборкаДетальныеЗаписи.План * 0.13;
	...
	Процедура РассчитатьОтпуск()
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			...
			Иначе
				...
				Движение.Результат = (ВыборкаДетальныеЗаписи.РезультатБазаОсновные + ВыборкаДетальныеЗаписи.РезультатБазаДополнительные) / ВыборкаДетальныеЗаписи.ОтработаноДней * ВыборкаДетальныеЗаписи.Показатель;
			...
			Движение.Сотрудник = ВыборкаДетальныеЗаписи.Сотрудник;
			Движение.КоличествоДней = ВыборкаДетальныеЗаписи.Показатель; 
			Движение.Период = Дата;
			
			//?
			//ГодОтпуска = Формат(Дата, "ДФ=yyyy");
			////ГодОтпуска = Год(Дата);
			...
			Движение.ПериодРегистрации = Дата;
			Движение.Сотрудник = ВыборкаДетальныеЗаписи.Сотрудник;
			Движение.Результат = ((ВыборкаДетальныеЗаписи.РезультатБазаОсновные + ВыборкаДетальныеЗаписи.РезультатБазаДополнительные) / ВыборкаДетальныеЗаписи.ОтработаноДней * ВыборкаДетальныеЗаписи.Показатель) * 0.13;
	...
	Процедура НачислитьПроцентЗаВыполненныеРаботы()
		...
		|ГДЕ
		|	ВКМ_НачислениеЗарплатыНачисления.Ссылка = &Ссылка
		|	И ВКМ_НачислениеЗарплатыНачисления.ВидРасчета = &Премия";
		
		Запрос.УстановитьПараметр("НачалоПериода", НачалоМесяца(Дата));
		Запрос.УстановитьПараметр("КонецПериода", КонецМесяца(Дата));
		Запрос.УстановитьПараметр("Ссылка", Ссылка);
		Запрос.УстановитьПараметр("Премия", ПланыВидовРасчета.ВКМ_ДополнительныеНачисления.ПроцентЗаВыполненныеРаботы);
